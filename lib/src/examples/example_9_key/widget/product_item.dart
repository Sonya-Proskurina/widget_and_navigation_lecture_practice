...
import 'package:flutter_widget_seminar/src/examples/example_9_key/navigation/navigation_manager.dart';
...
class _ProductRowItemState extends State<ProductRowItem> {
  ...
  Widget build(BuildContext context) => InkWell(
        onTap: () async {
          final count = ShopInheritedNotifier.of(context)
              .getValueNotifierById(widget.product.id)
              .value;

          /// открытие новой страницы
          final res = await NavigationManagerExample9.instance.openProduct(
            widget.product,
            count,
          );

          ShopInheritedNotifier.of(context).addCountById(
            widget.product.id,
            newValue: res,
          );
        },
        child: ProductItemInheritedNotifier(
